...
const authJsonWriteCmd = computed(() => {
  if (props.platform === 'windows') {
    return `New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\\.codex" | Out-Null; '{"OPENAI_API_KEY": "cr_xxxxxxxxxx"}' | Set-Content -Path "$env:USERPROFILE\\.codex\\auth.json" -Force`
  }
