...
// 资源包：统一按命名规则识别，新增面额时无需改数字列表
const RESOURCE_PACK_PATTERNS = [/_pack-\d+-month$/i, /pack-\d+-enp$/i]
const isResourcePackName = (name) =>
  RESOURCE_PACK_PATTERNS.some((pattern) => pattern.test(name || ''))

// ##target## / ##logPath## 为模板变量，变量位置使用通配以兼容替换后的真实内容。
const AI_FIX_PATTERNS = [
  /请修复[\s\S]+?控制台中的错误，修复后继续分析[\s\S]+?控制台日志，直到控制台没有错误。/,
  /请修复[\s\S]+?控制台中的错误。/,
  /当前项目运行到[\s\S]+?时出现内存泄漏问题，内存泄漏日志已输出(?:至)?\s*[:：]\s*[\s\S]+?，请读取该日志分析出内存泄漏的原因并修复。/,
  /当前项目在云打包时发生错误，错误日志已输出(?:至)?\s*[:：]\s*[\s\S]+?，请读取错误日志分析错误原因，然后给出解决方案。/
]
...
function isAiFixRequest(req) {
  ...
  return messages.some((m) => {
    ...
    return AI_FIX_PATTERNS.some((pattern) => pattern.test(text))
  })
}
...
async function checkApiKeyLimits(keyData, req, options = {}) {
  ...
  if (isAiFixPack(keyData.name)) {
    ...
    if (processType === 'primary' && !isAiFixRequest(req)) {
      return {
        valid: false,
        statusCode: 403,
        error: {
          code: 'ai_fix_pack_restricted',
          message: `AI修复 资源包仅限 AI修复 功能使用，如需继续使用请购买 [<a href="${subscriptionUrl}">订阅套餐</a>] 或 [<a href="${subscriptionUrl}">补充资源包</a>]。`
        }
      }
    }
  }
...
const authenticateApiKey = async (req, res, next) => {
  ...
  try {
    ...
    if (!apiKey || apiKey === 'Bearer') {
      ...
      if (earlyPackMode === 'enterprise') {
        return res.status(402).json({
          error: {
            type: 'insufficient_quota',
            message:
              '您没有企业版使用权限。企业版需由企业管理员（即贵公司在 DCloud 完成企业实名认证的账号负责人）购买并授权：<br/>1) 在 [<a href="https://dev.dcloud.net.cn">DCloud 开发者中心</a>] 购买企业版；<br/>2) 将您加入企业成员；<br/>3) 为您分配使用权限；<br />[<a href="https://doc.dcloud.net.cn/uni-app-x/ai/enterprise-subscription.html">查看企业版说明</a>]<br/>如您是个人用户，请切换到个人版继续使用。',
    ...
    if (packMode === 'enterprise') {
      ...
      // 注意：key 禁用/过期不拦截——与个人版切换逻辑一致，企业成员资格由 userId 控制
      ...
      const enterpriseResourcePacks = enterpriseCandidates.filter((k) => isResourcePackName(k.name))
      const enterpriseOthers = enterpriseCandidates.filter(
        (k) => !isPackageName(k.name) && !isResourcePackName(k.name)
      )
    ...
    // 个人版模式下禁止使用企业 key：标记为失败，触发后续 key 切换去找个人版 key
    if (
      packMode !== 'enterprise' &&
      validation.valid &&
      ...
      if (!validation.valid) {
        ...
        if (failedKeyData?.externalUid) {
          ...
          if (keyIds && keyIds.length > 0) {
            ...
            const resourcePacks = allKeys.filter((k) => isResourcePackName(k.name))
            const aiFixPacks = allKeys.filter((k) => isAiFixPack(k.name))
            const others = allKeys.filter(
              (k) => !isPackage(k.name) && !isResourcePackName(k.name) && !isAiFixPack(k.name)
            )
            ...
            const currentIsResourcePack =
              isResourcePackName(failedKeyData.name) || isAiFixPack(failedKeyData.name)
            logger.api(
        ...
        if (!validation.valid) {
          ...
          // 企业 key 在个人模式下使用且切换失败：按个人版提示引导用户补充个人额度
          if (
            validation.error?.code === 'quota_exhausted' &&
            failedKeyData?.packMode === 'enterprise'
          ) {
            return res.status(402).json({
              error: {
                type: 'insufficient_quota',
