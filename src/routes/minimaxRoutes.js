...
router.post('/v1/chat/completions', authenticateApiKey, async (req, res) => {
  try {
    ...
    return await minimaxRelayService.handleChatCompletions(req, res)
  } catch (error) {
    logger.error('❌ MiniMax chat/completions route error:', error)
    if (!res.headersSent) {
...
router.post('/anthropic/v1/messages', authenticateApiKey, async (req, res) => {
  try {
    ...
    return await minimaxRelayService.handleAnthropicMessages(req, res)
  } catch (error) {
    logger.error('❌ MiniMax Anthropic messages route error:', error)
    if (!res.headersSent) {
      return res.status(500).json({
        type: 'error',
        error: {
