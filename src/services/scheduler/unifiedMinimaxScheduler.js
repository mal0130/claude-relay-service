...
class UnifiedMiniMaxScheduler {
  constructor() {
    this.SESSION_MAPPING_PREFIX = 'minimax_session_mapping:'
  }
  ...
  async selectAccountForApiKey(apiKeyData, sessionHash = null, requestedModel = null) {
    if (sessionHash) {
      ...
      if (mapped && (await this._isAccountAvailable(mapped.accountId))) {
        ...
        await minimaxAccountService.markAccountUsed(mapped.accountId)
        logger.info(`🎯 Using sticky MiniMax account: ${mapped.accountId}`)
        return { accountId: mapped.accountId, accountType: 'minimax' }
      }
      if (mapped) {
    ...
    if (accounts.length === 0) {
      throw new Error('No available MiniMax accounts')
    }
    ...
    await minimaxAccountService.markAccountUsed(selected.id)
    logger.info(
      `🎯 Selected MiniMax account: ${selected.name} (${selected.id}) with priority ${selected.priority}`
    )

    return { accountId: selected.id, accountType: 'minimax' }
  }

  async _getAllAvailableAccounts(apiKeyData, requestedModel = null) {
    const binding = this._getMiniMaxBinding(apiKeyData)
    if (binding?.accountId) {
      const account = await minimaxAccountService.getAccount(binding.accountId)
    ...
    const accounts = await minimaxAccountService.getAllAccounts(false)
    const available = []
  ...
  async selectAccountFromGroup(
    ...
    if (accounts.length === 0) {
      throw new Error('No available MiniMax accounts in group')
    }
  ...
  async _isAccountUsable(account, requestedModel = null) {
    ...
    if (await upstreamErrorHelper.isTempUnavailable(account.id, 'minimax')) {
      return false
    }
    ...
    if (
      ...
      return minimaxAccountService.isModelSupported(account.supportedModels, requestedModel)
    }
  ...
  async isAccountRateLimited(accountId) {
    ...
    const cleared = await minimaxAccountService.checkAndClearRateLimit(accountId)
    return !cleared
  }

  async markAccountRateLimited(accountId, sessionHash = null, duration = null) {
    await minimaxAccountService.setAccountRateLimited(accountId, true, duration)
    if (sessionHash) {
      await this.clearSessionMapping(sessionHash)
    }
  }
  ...
  _getMiniMaxBinding(apiKeyData = {}) {
    ...
    if (minimax.accountId) {
      if (String(minimax.accountId).startsWith('group:')) {
        return { groupId: String(minimax.accountId).replace(/^group:/, '') }
      }
      return { accountId: minimax.accountId }
    }

    if (minimax.groupId) {
      return { groupId: minimax.groupId }
    }
  ...
  async _setSessionMapping(sessionHash, accountId) {
    ...
    await redis
      ...
      .setex(
        ...
        ttl,
        JSON.stringify({ accountId, accountType: 'minimax', createdAt: new Date().toISOString() })
      )
  }
...
module.exports = new UnifiedMiniMaxScheduler()
