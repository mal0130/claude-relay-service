...
const { GLM_PLATFORM, GLM_DEFAULT_BASE_API, normalizeBaseApi } = require('../glmPlatform')

const encryptor = createEncryptor('glm-api-salt')

class GlmAccountService {
  constructor() {
    this.ACCOUNT_KEY_PREFIX = 'glm:account:'
    this.ACCOUNT_INDEX_KEY = 'glm_account:index'
    this.SHARED_ACCOUNTS_KEY = 'shared_glm_accounts'
  }
  ...
  async createAccount(options = {}) {
    ...
    if (!apiKey) {
      throw new Error('API Key is required for GLM account')
    }
    ...
    const accountData = {
      ...
      platform: 'glm',
      accountType,
      accountSubType: GLM_PLATFORM.accountSubType,
      name,
      description,
    ...
    logger.success(`Created GLM account: ${name} (${accountId})`)
  ...
  async updateAccount(accountId, updates = {}) {
    ...
    logger.info(`📝 Updated GLM account: ${account.name}`)
    return { success: true }
  }
  ...
  async deleteAccount(accountId) {
    ...
    await upstreamErrorHelper.clearTempUnavailable(accountId, 'glm').catch(() => {})
    logger.info(`🗑️ Deleted GLM account: ${accountId}`)
    return { success: true }
  }
  ...
  async getAllAccounts(includeInactive = false) {
    ...
    for (const accountData of dataList) {
      ...
      accountData.apiKey = '***'
      accountData.baseApi = normalizeBaseApi(accountData.baseApi)
      accountData.platform = accountData.platform || 'glm'
      accountData.accountSubType = accountData.accountSubType || GLM_PLATFORM.accountSubType
  ...
  async setAccountRateLimited(accountId, isLimited, duration = null) {
    ...
    if (isLimited) {
      if (account.disableAutoProtection === true || account.disableAutoProtection === 'true') {
        logger.info(`🛡️ GLM account ${accountId} disabled auto-protection, skip rate limit`)
        upstreamErrorHelper.recordErrorHistory(accountId, 'glm', 429, 'rate_limit').catch(() => {})
        return
      }
      ...
      logger.warn(
        `⏳ GLM account ${account.name} marked rate limited for ${rateLimitDuration} minutes`
      )
    } else {
      await this.updateAccount(accountId, {
        rateLimitedAt: '',
      ...
      logger.info(`✅ Rate limit cleared for GLM account ${account.name}`)
    }
  }

  async markAccountUnauthorized(accountId, reason = 'GLM账号认证失败') {
    const account = await this.getAccount(accountId)
    if (!account) {
      return
    }
    ...
    if (account.disableAutoProtection === true || account.disableAutoProtection === 'true') {
      logger.info(`🛡️ GLM account ${accountId} disabled auto-protection, skip unauthorized`)
      upstreamErrorHelper.recordErrorHistory(accountId, 'glm', 401, 'auth_error').catch(() => {})
      return
    }
    ...
    logger.warn(`🚫 GLM account ${account.name || accountId} marked unauthorized`)
  }
...
module.exports = new GlmAccountService()
