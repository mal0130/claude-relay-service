...
const { KIMI_PLATFORM, KIMI_DEFAULT_BASE_API, normalizeBaseApi } = require('../kimiPlatform')

const encryptor = createEncryptor('kimi-api-salt')

class KimiAccountService {
  constructor() {
    this.ACCOUNT_KEY_PREFIX = 'kimi:account:'
    this.ACCOUNT_INDEX_KEY = 'kimi_account:index'
    this.SHARED_ACCOUNTS_KEY = 'shared_kimi_accounts'
  }
  ...
  async createAccount(options = {}) {
    ...
    if (!apiKey) {
      throw new Error('API Key is required for Kimi account')
    }
    ...
    const accountData = {
      ...
      platform: 'kimi',
      accountType,
      accountSubType: KIMI_PLATFORM.accountSubType,
      name,
      description,
    ...
    logger.success(`Created Kimi account: ${name} (${accountId})`)
  ...
  async updateAccount(accountId, updates = {}) {
    ...
    logger.info(`📝 Updated Kimi account: ${account.name}`)
    return { success: true }
  }
  ...
  async deleteAccount(accountId) {
    ...
    await upstreamErrorHelper.clearTempUnavailable(accountId, 'kimi').catch(() => {})
    logger.info(`🗑️ Deleted Kimi account: ${accountId}`)
    return { success: true }
  }
  ...
  async getAllAccounts(includeInactive = false) {
    ...
    for (const accountData of dataList) {
      ...
      accountData.apiKey = '***'
      accountData.baseApi = normalizeBaseApi(accountData.baseApi)
      accountData.platform = accountData.platform || 'kimi'
      accountData.accountSubType = accountData.accountSubType || KIMI_PLATFORM.accountSubType
  ...
  async setAccountRateLimited(accountId, isLimited, duration = null) {
    ...
    if (isLimited) {
      if (account.disableAutoProtection === true || account.disableAutoProtection === 'true') {
        logger.info(`🛡️ Kimi account ${accountId} disabled auto-protection, skip rate limit`)
        upstreamErrorHelper.recordErrorHistory(accountId, 'kimi', 429, 'rate_limit').catch(() => {})
        return
      }
      ...
      logger.warn(
        `⏳ Kimi account ${account.name} marked rate limited for ${rateLimitDuration} minutes`
      )
    } else {
      await this.updateAccount(accountId, {
        rateLimitedAt: '',
      ...
      logger.info(`✅ Rate limit cleared for Kimi account ${account.name}`)
    }
  }

  async markAccountUnauthorized(accountId, reason = 'Kimi账号认证失败') {
    const account = await this.getAccount(accountId)
    if (!account) {
      return
    }
    ...
    if (account.disableAutoProtection === true || account.disableAutoProtection === 'true') {
      logger.info(`🛡️ Kimi account ${accountId} disabled auto-protection, skip unauthorized`)
      upstreamErrorHelper.recordErrorHistory(accountId, 'kimi', 401, 'auth_error').catch(() => {})
      return
    }
    ...
    logger.warn(`🚫 Kimi account ${account.name || accountId} marked unauthorized`)
  }
...
module.exports = new KimiAccountService()
